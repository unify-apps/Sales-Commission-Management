...
async function fetchLatestSkillVersion() {
  // Update-check network call removed — skills make no external calls (it hung the
  // agent behind the sandbox's TLS proxy, and the skill can't self-update anyway).
  return null;
}
