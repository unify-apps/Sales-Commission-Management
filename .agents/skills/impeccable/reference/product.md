...
The floor is a *deliberate, consistent* palette — commit to a scheme on purpose and execute it precisely across every surface. The failure mode is an undecided palette (random accents, mismatched surfaces, timid-by-default), not a bold one.

Choose the commitment level from the product's character, not a default:
- **Restrained** (tinted neutrals + one accent) suits dense, neutral-canvas tools where the data is the star.
- **Committed** is equally valid and often stronger for products with their own identity: a confident dark surface, one category color carrying a dashboard, a saturated developer / analytics / ops aesthetic. Don't downgrade a deliberate bold choice to restrained just to play safe.
- **Dark vs light is a deliberate decision in either direction** — pick for the product's context and audience; never default to light "to be safe" or dark "to look cool."
