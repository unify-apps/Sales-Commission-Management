...
Browser visualization is helpful for a viewable target when browser automation is available — seeing the rendered page catches visual issues the CLI detector can't. Use a localhost dev/static URL for local files; avoid `file://` unless the available browser explicitly supports this workflow. Flow:

1. Create a fresh tab and navigate.
2. If the browser can render the target, present it, label `[Human]`, and scroll top for a visual review pass — inspect hierarchy, spacing, contrast, and rendered slop the CLI detector can't see.
3. If it can't be rendered, skip browser presentation and report a fallback signal.
4. For multi-view targets, review 3-5 representative pages.

Codex Browser note: Use the Browser skill. Do not spend a Browser attempt on `file://`. Call `visibility.set(true)` to present the page for the `[Human]` visual pass; verify with `get()`. Its Playwright `evaluate(...)` surface is read-only — fine for a visual review.

Return: CLI findings JSON/counts, visual review notes if the page was rendered, false positives, and skipped/failed browser steps with concrete reasons.
...
Codex failure accounting: final Run Notes must include target slug, ignore list, assessment independence, CLI detector, browser visibility, temp-file cleanup, and any fallback signal used. Do not run repo status checks, late API spelunking, or unrelated verification after the report is assembled.
...
**Visual review** (if the page was rendered): Summarize what you saw in the **[Human]** tab — the visual issues the CLI detector can't catch. If browser visualization was attempted but the page couldn't render, say so and report the fallback signal instead.
...
#### Run Notes
Keep this compact. Include status for target slug, ignore list, assessment independence, CLI detector, browser visibility, and temp-file cleanup. For failed or skipped steps, give the concrete observed reason and the fallback signal used. In the final chat response, also include snapshot write and trend read status after persistence has run.
